<script setup>
    ...
    import { ref } from 'vue';
    import { fetchPendingCourseData } from './services/fetchPendingCoursesData.js';
    ...
    // Cargar los cursos pendientes al montar el componente
    onMounted(async () => {
        try {
            ...
            courses.value = await fetchPendingCourseData('', collaboratorId);
    ...
    // Filtrar los cursos pendientes por busqueda (nombre)
    async function filterCourses() {
        try {
            const filteredCourses = await fetchPendingCourseData(searchTerm.value);
...
<template>
    ...
    <PanelLayout title="Dashboard">
        ...
        <Table :Items="courses" :Categorie="'pendingCourses'" />
    </PanelLayout>
